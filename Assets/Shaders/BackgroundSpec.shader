Shader "Custom/BackgroundSpec(Obsolete)" {
    Properties {
        _Color ("Main Color", Color) = (1,1,1,1)
        _SpecMap ("Specular Map", 2D) = "white" {}  // New specular map
    ...
    SubShader {
        ...
        Pass {
            ...
            sampler2D _MainTex;
            sampler2D _BumpMap;
            sampler2D _SpecMap; // Sampler for the specular map
            float4 _Color;
            float _BumpPower;
            ...
            half4 frag(v2f i) : SV_Target {
                ...
                float4 specMap = tex2D(_SpecMap, i.uv);  // Sampling the specular map
                ...
                float spec = pow(max(dot(viewDir, reflectDir), 0.0), specMap.a); // Use alpha for shininess
                float3 specular = specMap.rgb * _LightColor0.rgb * spec;  // Use RGB for specular intensity
